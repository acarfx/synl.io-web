<?php
    ...
    if ($URL != "synl.io/synl/pr/index.php")
    die("/\*yarramı alırsın amcık xd*/");
?>
...
async function addRow(settings, id, name, puan) {
    ...
    if(settings == "_staffs") {
        ...
        td3.innerText = `${puan}`;
        td4.innerHTML = `<button type="button" onClick="deleteRow('${settings}', '${id}', this);" class="btn btn-danger">Kaldır</button>`;
    ...
    td1.innerText = `#${kanal[0].name}`;
    td2.innerText = `${name}`;
    td3.innerHTML = `<button type="button" onClick="deleteRow('${settings}', '${id}', this);" class="btn btn-danger">Kaldır</button>`;
    postData(`${API.url}/push`, {
        name: settings,
        id: id, 
        isim: name
...
const GetStaffs = async () => {
    ...
    if(chatdata) {
        ...
        chatdata.value.sort((a, b) => a.No - b.No).filter(x => GetRole(x.rol)).map(async (x, index) => {
            ...
            td4.innerHTML = `<button type="button" onClick="deleteRow('_staffs', '${x.rol}', this);" class="btn btn-danger">Kaldır</button>`;
        })
    } else {
        console.log({
            status: "ERROR",
...
const GetVoices = async () => {
    ...
    if(chatdata) {
        ...
        chatdata.value.filter(x => GetChannel(x.id)).map(async (x, index) => {
            ...
            td3.innerHTML = `<button type="button" onClick="deleteRow('_voices', '${x.id}', this);" class="btn btn-danger">Kaldır</button>`;
        })
    } else {
        console.log({
            status: "ERROR",
...
const GetChats = async () => {
    ...
    if(chatdata) {
        ...
        chatdata.value.filter(x => GetChannel(x.id)).map(async (x, index) => {
            ...
            td1.innerText = `#${kanal[0].name}`;
            td2.innerText = `${x.isim}`;
            td3.innerHTML = `<button type="button" onClick="deleteRow('_chats', '${x.id}', this);" class="btn btn-danger">Kaldır</button>`;
        })
    } else {
        console.log({
            status: "ERROR",
...
<label class="custom-switch-btn" for="${item.name}_d" style="align-items: left;"></label>
        ...
        td3.innerHTML = `<button type="button" id="${item.name}" onclick="update('${item.name}', '${item.type}', false, false, true)"  name="güncelle" class="btn btn-secondary">Güncelle</button>`;
       if(item.type == "roller" || item.type == "kanallar" || item.type == "limit"){
...
function update(itemname, type, sıfırla = false, stat = false, guard = false) {
    ...
    if(type == "cogul") {
        ...
        if(sıfırla) cogul = []
        postData(`${API.url}/post`, {name: itemname, var: cogul})
        get.value = ''
...
async function checkServer() {
    ...
    _result.charts.filter(x => (new Date().getTime() - x.tarih) < 604800000).slice(0, 7).map(x => {
